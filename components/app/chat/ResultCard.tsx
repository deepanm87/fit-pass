...
function ClassCard({
  ...
  return (
    <Link href={`/classes?${searchParams.toString()}`} onClick={onClick} className={cardClasses}>
      <div className="flex size-10 shrink-0 items-center justify-center rounded-lg bg-primary/10 transition-colors duration-200 group-hover:bg-primary/20">
        <Dumbbell className="size-5 text-primary" />
      </div>
      <div className="min-w-0 flex-1">
        <div className="flex items-start justify-between gap-2">
          <div className="min-w-0">
            <span className="block truncate text-sm font-medium text-foreground transition-colors duratiion-200 group-hover:text-primary">
              {data.name}
            </span>
            {data.category?.name && (
...
function SessionCard({
  ...
  const isLowSports = data.spotsAvailable > 0 && data.spotsAvailable <= 3
...
function BookingCard({
  ...
  return (
    ...
    >
      ...
      >
        <StatusIcon 
          className={`size-5 ${isPast ? "text-muted-foreground" : "text-emerald-6-- dark:text-emerald-400"}`}
        />
      </div>
      <div className="min-w-0 flex-1">
        <div className="flex items-start justify-between gap-2">
          <div className="min-w-0">
            <span className={`block truncate text-sm font-medium transition-colors duration-200 ${
              isPast
