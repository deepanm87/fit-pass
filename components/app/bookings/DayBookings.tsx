...
export function DayBookings({ bookings, selectedDate }: DayBookingsProps) {
  const dayBookings = bookings.filter(booking => {
    ...
    if (!isSameDay(classTime, selectedDate)) {
      return false
    }
      ...
      <div className="flex h-full flex-col items-center justify-center rounded-lg border bg-card p-6 text-center">
        ...
        <p className="text-sm text-muted-foreground">
          {isTodaySelected
            ...
            : `No classes booked for ${format(selectedDate, "MMM d")}`
          }
        </p>
        <Link
          href="/classes"
  ...
  return (
    <div className="space-y-3">
      <h3 className="text-sm font-medium text-muted-foreground">
        {format(selectedDate, "EEE MMM d")} · {dayBookings.length}{" "}
            ...
            >
              ...
              <div className="min-w-0 flex-1">
                <div className="flex items-start justify-between gap-2">
                  ...
                  <span className={`shrink-0 rounded-full px-2 py-0.5 text-xs font-medium ${BOOKING_STATUS_COLORS[effectiveStatus] || BOOKING_STATUS_COLORS.confirmed}`}>
                ...
                <div className="mt-1 flex flex-wrap items-center gap-x-3 gap-y-1 text-sm text-muted-foreground">
                  <span className="flex items-center gap-1">
                    ...
                    {format(startTime, "hh:mm a")} · {duration} min
                  </span>
                  <span className="flex items-center gap-1">
