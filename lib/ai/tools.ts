...
export const getClassSessions = tool({
  description: "Get upcoming scheduled sessions for a specific class or activity. Show dates, times, venues, and availability.",
  inputSchema: z.object({
    className: z
      .string()
...
export const searchVenues = tool({
  description: "Search for fitness venues/studios by name or city. Returns venue details including address and amenities.",
  inputSchema: z.object({
...
export const getCategories = tool({
  description: "Get all available fitness class categories. Useful when users want to know what type of classes are offered.",
  inputSchema: z.object({}),
  execute: async () => {
...
export const getSubscriptionInfo = tool({
  description: "Get information about subscription tiers, pricing, and what classes each tier can access.",
  inputSchema: z.object({}),
  execute: async () => {
    return {
      tiers: [
...
export const getRecommendations = tool({
  description: "Get personalized class recommendations based on user preferences like fitness goals, preferred time of day, or difficulty level. Only returns classes with upcoming sessions available.",
  inputSchema: z.object({
    fitnessGoal: z
...
export const getUserBookings = tool({
  description: "Get the current user's bookings. Can filter by upcoming or past bookings. Use this when users ask about their scheduled classes, booking history, or want to know what classes they have coming up. The clerkId is provided in the system context.",
  inputSchema: z.object({
    type: z
      .enum(["upcoming", "past", "all"])
      .optional()
      .describe("Filter bookings: 'upcoming' for future classes, 'past' for completed, 'all' for everything. Defaults to 'upcoming'."),
      clerkId: z
        .string()
        .describe("The user's Clerk ID from the system context. Extract this from the system message.")
  }),
    ...
    return {
      ...
      bookings: bookings.map(
        (b: any) => ({
          ...
          dateTime: b.classSession?.venue?.name,
